...
public class FirstPersonDrawer extends DefaultViewer {
	 ...
	 * @param panel the MazePanel for this class
	...
	public void redraw(MazePanel panel, StateGUI state, int px, int py, int view_dx, int view_dy, int walk_step, int view_offset, RangeSet rset, int ang) {
		...
		if (state != StateGUI.STATE_PLAY) return;
		...
		panel.setColor(Colors.WHITE, true);
	...
	private void drawBackground() {
		panel.setColor(Colors.BLACK, true);
		panel.fillRect(0, 0, view_width, view_height/2);
		panel.setColor(Colors.DARK_GRAY, true);
	...
	private void drawSegment(Seg seg) {
		...
		int x1i = x1;
		int xd = x2-x1;
		panel.setColor(seg.getColor(), true);
		boolean drawn = false;
